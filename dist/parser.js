...
import { BinOp, UniOp, Type } from "./ast";
import { organizeProgram } from "./form";
export function traverseExpr(c, s) {
    ...
    switch (c.type.name) {
        ...
        case "UnaryExpression": {
            ...
            let unaryOp = s.substring(c.from, c.to);
            c.nextSibling(); //traverse to the target expression
            let targetExpr = traverseExpr(c, s);
        ...
        case "BinaryExpression": {
            ...
            let rightExpr = traverseExpr(c, s);
        ...
        case "ParenthesizedExpression": {
            ...
            c.nextSibling(); //skip "("
            let nestedExpr = traverseExpr(c, s);
            c.parent();
        ...
        case "CallExpression": {
            ...
            let unknownIfArg = s.substring(c.from, c.to);
    ...
    switch (c.node.type.name) {
        case "AssignStatement": {
            ...
            if (c.node.type.name === "TypeDef") {
                ...
                switch (lvarTypeName) {
                    case Type.Int: {
                        localVarType = Type.Int;
                        break;
                    }
                    case Type.Bool: {
                        localVarType = Type.Bool;
                        break;
                    }
            ...
            let lvarValue = traverseExpr(c, s);
            c.parent();
        ...
        case "FunctionDefinition": {
            ...
            while (s.substring(c.from, c.to) !== ")") {
                ...
                if (s.substring(c.from, c.to) !== ",") {
                    ...
                    if (expectName) {
                        ...
                        let tempParamType = s.substring(c.from, c.to);
                        expectName = true;
                        switch (tempParamType) {
                            case Type.Int: {
                                params.set(tempParamName, Type.Int);
                                break;
                            }
                            case Type.Bool: {
                                params.set(tempParamName, Type.Bool);
                                break;
                            }
            ...
            if (c.node.type.name === "TypeDef") {
                ...
                let rawReturnType = s.substring(c.from, c.to);
            ...
            let funcLocalVars = new Map();
            let funcStates = new Array;
        ...
        case "IfStatement": {
            ...
            let firstIfCond = traverseExpr(c, s);
            c.nextSibling(); //goes to the body
            c.firstChild(); //goes to the starting colon of the body
            let firstIfBody = new Array;
            while (c.nextSibling()) {
            ...
            while (c.nextSibling()) {
                ...
                switch (c.node.type.name) {
                    case "elif": {
                        ...
                        let elifCondition = traverseExpr(c, s);
...
export function parse(source) {
    ...
    let stmts = traverse(t.cursor(), source);
